  ...
  function submitLogin(){
    ...
    (async () => {
      ...
      if(u){
        ...
        gtag('set', 'user_properties', { user_type: 'personal' });
      } else {
  ...
  function parseWorkHoursStr(s){ if(!s) return 0; const m=s.match(/(\d{1,2}):(\d{2})\s*[–\-]\s*(\d{1,2}):(\d{2})/); if(!m) return 0; const start=parseInt(m[1])+parseInt(m[2])/60; const end=parseInt(m[3])+parseInt(m[4])/60; const diff=end-start; return diff>0?diff:0; }
  function getLonRollvikt(task,settings){ const s=settings||loadWageSettings(); const v=Number((s.task_vikter||{})[task||'']); return (v&&v>0)?v:1.0; }
  function getLonTransaktionerForPeriod(fromDate,toDate){ const hist=loadSalesHistory(); const counts={}; const d=new Date(fromDate+'T12:00:00'); const end=new Date(toDate+'T12:00:00'); while(d<=end){ const ds=d.toISOString().slice(0,10); const entries=(hist[ds]||{}).entries||[]; entries.forEach(e=>{ const s=e.seller||''; if(s&&s!=='—') counts[s]=(counts[s]||0)+1; }); d.setDate(d.getDate()+1); } return counts; }
  function getLonIntakterForPeriod(fromDate,toDate){ const hist=loadSalesHistory(); const sums={}; const d=new Date(fromDate+'T12:00:00'); const end=new Date(toDate+'T12:00:00'); while(d<=end){ const ds=d.toISOString().slice(0,10); const entries=(hist[ds]||{}).entries||[]; entries.forEach(e=>{ const s=e.seller||''; if(s&&s!=='—') sums[s]=(sums[s]||0)+Number(e.sum||0); }); d.setDate(d.getDate()+1); } return sums; }

  function getLonUgifterForPeriod(fromDate,toDate){ const hist=loadSalesHistory(); let total=0; const d=new Date(fromDate+'T12:00:00'); const end=new Date(toDate+'T12:00:00'); while(d<=end){ const ds=d.toISOString().slice(0,10); total+=Number((hist[ds]||{}).costs||0); d.setDate(d.getDate()+1); } return total; }
  ...
  function renderSchemaForEmployee(){
    ...
    const todayOpenEntry=allOpenDates.find(e=>e.date===today);
    const todayOpenHrs=todayOpenEntry?.hours||openingHours||'';
    const savedToday=userData[name]?.hoursDate===today;
    const todayPlanned=plan[today]?.[name]||(savedToday?{hours:userData[name]?.hours||'',task:userData[name]?.task||''}:{hours:'',task:''});
    ...
    el.innerHTML=todayHtml+future.map(e=>{
      ...
      return dayRow(e.date,e.hours,planned.hours,planned.task,false);
    }).join('')+pastHtml;
  }
  ...
  function saveSchemaForEmployee(){
    ...
    document.querySelectorAll('.schema-hours[data-date]').forEach(inp=>{
      ...
      if(inp.dataset.isToday){
        ...
        saveUserData(); snapshotStaffHoursToday();
        if(loggedInUser===name) loadProfile();
      }
    });
    saveShiftPlan(plan);
  ...
  function getLonStaffDataForPeriod(fromDate,toDate){
    const hist=loadSalesHistory(); const shiftPlan=loadShiftPlan(); const today=todayStr();
    const acc={}; let hasMissingSnapshot=false;
    const d=new Date(fromDate+'T12:00:00'); const end=new Date(toDate+'T12:00:00');
    while(d<=end){
      const ds=d.toISOString().slice(0,10);
      let daySnap=null; let isFallback=false;
      if(ds===today){ daySnap={}; Object.keys(userData).forEach(n=>{daySnap[n]={hours:userData[n]?.hours||'',task:userData[n]?.task||''};});}
      else if((hist[ds]||{}).staffHours){ daySnap=hist[ds].staffHours; }
      else if(shiftPlan[ds]){ daySnap=shiftPlan[ds]; }
      else{ isFallback=true; hasMissingSnapshot=true; daySnap={}; Object.keys(userData).forEach(n=>{daySnap[n]={hours:userData[n]?.hours||'',task:userData[n]?.task||''};});}
      Object.keys(daySnap).forEach(name=>{
        const hrs=parseWorkHoursStr(daySnap[name].hours||'');
        if(!acc[name]) acc[name]={totalHrs:0,taskCounts:{}};
        acc[name].totalHrs=+(acc[name].totalHrs+hrs).toFixed(2);
        const t=daySnap[name].task||'—'; acc[name].taskCounts[t]=(acc[name].taskCounts[t]||0)+1;
      });
      d.setDate(d.getDate()+1);
    }
    const result=Object.keys(acc).map(name=>{ const task=Object.entries(acc[name].taskCounts).sort((a,b)=>b[1]-a[1])[0]?.[0]||'—'; return {name,totalHrs:acc[name].totalHrs,task}; }).filter(s=>s.totalHrs>0);
    return {staff:result,hasMissingSnapshot};
  }
  ...
  function renderLonHistorik(){
    const el=document.getElementById('lonHistorikList'); if(!el) return;
    const hist=loadSalesHistory();
    const today=todayStr();
    const days=Object.keys(hist).filter(d=>d<today&&!hist[d]?._deleted&&Number((hist[d]||{}).income||0)>0).sort((a,b)=>b.localeCompare(a));
    if(!days.length){ el.innerHTML='<div class="muted" style="margin-top:8px;">Inga tidigare öppna dagar med inkomst hittades.</div>'; return; }
    const settings=loadWageSettings();
    const bassumma=Math.max(0,Number(settings.bassumma||0));
    const verkAndel=Math.max(0,Math.min(100,Number(settings.verksamhetsandel||0)))/100;
    el.innerHTML=days.map(date=>{
      const day=hist[date]||{};
      const income=Number(day.income||0);
      const verkDel=Math.round(income*verkAndel);
      const lonPott=income-verkDel;
      const hasSnap=!!(day.staffHours&&Object.keys(day.staffHours).length);
      let staffHtml='';
      if(hasSnap){
        const staffRaw=Object.entries(day.staffHours).map(([name,v])=>({name,hours:(v.hours||''),task:(v.task||'—')}));
        const staff=staffRaw.map(s=>({...s,hrs:parseWorkHoursStr(s.hours)})).filter(s=>s.hrs>0);
        const openHrs=Number((day.openingHrs||0));
        const dayTrans=(day.entries||[]).reduce((acc,e)=>{ const sel=e.seller||''; if(sel&&sel!=='—') acc[sel]=(acc[sel]||0)+1; return acc; },{});
        const dayTotalTrans=Object.values(dayTrans).reduce((a,b)=>a+b,0);
        const dayIntakt=(day.entries||[]).reduce((acc,e)=>{ const sel=e.seller||''; if(sel&&sel!=='—') acc[sel]=(acc[sel]||0)+Number(e.sum||0); return acc; },{});
        const dayTotalIntakt=Object.values(dayIntakt).reduce((a,b)=>a+b,0);
        const isVoluntarH=(name)=>Object.values(users).some(u=>u.name===name&&u.isVoluntar);
        staff.forEach(s=>{ s.isVoluntar=isVoluntarH(s.name); const baseVikt=getLonRollvikt(s.task,settings); const hasKB=!!(settings.task_kassaboost||{})[s.task||'']; if(!s.isVoluntar&&hasKB&&dayTotalTrans>0){ s.vikt=Math.min(1.0,+(baseVikt+(((dayTrans[s.name]||0)/dayTotalTrans)*(1.0-baseVikt))).toFixed(3)); } else { s.vikt=baseVikt; } s.kassaBoost=hasKB; s.personIntakt=dayIntakt[s.name]||0; s.tackning=openHrs>0?s.hrs/openHrs:0; s.poang=+(s.tackning*s.vikt).toFixed(4); });
        const normalStaffH=staff.filter(s=>!s.isVoluntar);
        const voluntarKassaH=staff.filter(s=>s.isVoluntar&&s.kassaBoost&&dayTotalIntakt>0&&s.personIntakt>0);
        const totalPoang=normalStaffH.reduce((a,s)=>a+s.poang,0);
        const totalBas=bassumma*normalStaffH.length;
        const deln=Math.max(0,lonPott-totalBas);
  ...
  function renderLon(opts){
    opts=opts||{};
    const fromVal=opts.fromVal||document.getElementById('lonFromDate').value;
    const toVal=opts.toVal||(document.getElementById('lonToDate').value||fromVal);
    const elId=opts.targetElId||'lonResult';
    const el=document.getElementById(elId);
    if(!el) return;
    if(!fromVal){el.innerHTML='<div class="muted" style="margin-top:10px;">Välj ett datum.</div>';return;}
    const settings=loadWageSettings();
    const bassumma=Math.max(0,Number(settings.bassumma||0));
    const verkAndel=Math.max(0,Math.min(100,Number(settings.verksamhetsandel||0)))/100;
    const skatteProcent=Math.max(0,Math.min(100,Number(settings.skatteprocent??50)))/100;
    const dayCount=getLonDaysBetween(fromVal,toVal);
    const totalIncome=getLonTotalIncomeForPeriod(fromVal,toVal);
    const totalUgifter=getLonUgifterForPeriod(fromVal,toVal);
    const skatteUnderlag=Math.max(0,totalIncome-totalUgifter);
    const skatt=Math.round(skatteUnderlag*skatteProcent);
    const nettoAttFordela=skatteUnderlag-skatt;
    const verkDel=Math.round(nettoAttFordela*verkAndel);
    const lonPott=nettoAttFordela-verkDel;
    const totalOpeningHrs=getLonOpeningHrsForPeriod(fromVal,toVal);
    const {staff:staffData,hasMissingSnapshot}=getLonStaffDataForPeriod(fromVal,toVal);
    const transaktioner=getLonTransaktionerForPeriod(fromVal,toVal);
    const totalTrans=Object.values(transaktioner).reduce((a,b)=>a+b,0);
    const intakter=getLonIntakterForPeriod(fromVal,toVal);
    const totalIntakt=Object.values(intakter).reduce((a,b)=>a+b,0);
    staffData.forEach(s=>{
      const baseVikt=getLonRollvikt(s.task,settings);
      s.personTrans=transaktioner[s.name]||0;
      s.personIntakt=intakter[s.name]||0;
      if((settings.task_kassaboost||{})[s.task||'']&&totalTrans>0){ s.vikt=Math.min(1.0,+(baseVikt+((s.personTrans/totalTrans)*(1.0-baseVikt))).toFixed(3)); s.kassaBoost=true; }
      else { s.vikt=baseVikt; s.kassaBoost=false; }
      s.tackning=totalOpeningHrs>0?s.totalHrs/totalOpeningHrs:0;
      s.poang=+(s.tackning*s.vikt).toFixed(4);
    });
    if(!staffData.length){el.innerHTML='<div class="muted" style="margin-top:12px;">Ingen personal med arbetstid eller gästinteraktion för perioden.</div>';return;}
    const isVoluntar=(name)=>Object.values(users).some(u=>u.name===name&&u.isVoluntar);
    staffData.forEach(s=>{ s.isVoluntar=isVoluntar(s.name); });
    const normalStaff=staffData.filter(s=>!s.isVoluntar);
    const voluntarKassa=staffData.filter(s=>s.isVoluntar&&s.kassaBoost&&totalIntakt>0&&s.personIntakt>0);
    const totalBas=bassumma*normalStaff.length;
    const delningsDel=Math.max(0,lonPott-totalBas);
    const voluntarRows=voluntarKassa.map(s=>{ const andel=totalIntakt>0?s.personIntakt/totalIntakt:0; const totalt=Math.round(andel*delningsDel); return {...s,andel,variabel:totalt,totalt}; });
    const totalVoluntarKassa=voluntarRows.reduce((a,r)=>a+r.totalt,0);
    const normalDelningsDel=Math.max(0,delningsDel-totalVoluntarKassa);
    const totalPoang=normalStaff.reduce((s,x)=>s+x.poang,0);
    const normalRows=normalStaff.map(s=>{ const andel=totalPoang>0?(s.poang/totalPoang):0; const variabel=Math.round(andel*normalDelningsDel); const totalt=bassumma+variabel; return {...s,andel,variabel,totalt}; });
    const rows=[...normalRows,...voluntarRows];
    ...
    html+=`<th style="text-align:center;padding:6px 8px;font-weight:normal;">Andel</th>`;
    ...
    if(totalOpeningHrs>0) html+=`<div class="tiny muted" style="margin-top:8px;">Andelen baseras på hur stor del av öppettiden (${totalOpeningHrs}h) varje person täckte.</div>`;
    if(hasMissingSnapshot) html+=`<div class="tiny muted" style="margin-top:4px;color:rgba(220,180,80,0.6);">Obs: vissa datum saknar sparade arbetstider — nuvarande inställningar används som ersättning.</div>`;
    el.innerHTML=html;
  }

  function renderLonIdag(){ const today=todayStr(); renderLon({fromVal:today,toVal:today,targetElId:'lonResultIdag'}); }
  ...
  function showLocalBackupModal(){
    ...
    const _hasData=d=>{ const v=localData[d]; return v&&!v._deleted&&(Number(v.income||0)>0||(Number(v.adult||0)+Number(v.child||0))>0); };
    const localDates=Object.keys(localData).filter(_hasData).sort();
    const serverData=loadSalesHistory();
    const serverDates=Object.keys(serverData).filter(d=>!serverData[d]?._deleted).sort();
    const missing=localDates.filter(d=>!serverData[d]||serverData[d]._deleted);
    window._backupMissing={dates:missing,localData};
  ...
  function restoreFromLocalBackup(){
    ...
    _statsDirty=true;
    const m=document.getElementById('localBackupModal');
    if(m) m.style.display='none';
    renderStats(); renderCockpit();
    showUndoToast(`${missing.length} dag${missing.length>1?'ar':''} återställda från lokal kopia`, ()=>{ const h=loadSalesHistory(); missing.forEach(d=>{ delete h[d]; }); saveSalesHistory(h); _statsDirty=true; renderStats(); renderCockpit(); });
  }
  ...
  function showReviewView(){
    ...
    root.innerHTML = `<div class="review-inner">
      <img src="korpen.png" class="review-logo" onerror="this.style.display='none'" alt="">
  ...
  function renderCatalogList(rootEl,list,kind){ if(!rootEl) return;if(!Array.isArray(list)||!list.length){rootEl.innerHTML='<div class="muted">Inga poster.</div>';return;} rootEl.innerHTML=list.map((name,idx)=>`<div style="margin:6px 0;"><div class="row" style="justify-content:space-between;gap:8px;"><input class="input ${kind}-name" data-index="${idx}" value="${escapeHtml(name)}" style="flex:1;min-width:180px;"><div class="row"><button class="btn save-${kind}" data-index="${idx}">Spara</button><button class="btn btn-danger del-${kind}" data-index="${idx}">Ta bort</button></div></div></div>`).join(''); }
  ...
  function renderTaskDescriptionsPanel(){
  ...
  function buildAdminSkillsOptions(userName){ const wrap=document.getElementById('adminSkillsOptions');if(!wrap) return;wrap.innerHTML='';const catalog=loadSkillCatalog();ensureProfileDefaults(userName);const selected=userData[userName].skills||[];catalog.forEach(s=>{const checked=selected.includes(s)?'checked':'';const lbl=document.createElement('label');lbl.className='row';lbl.innerHTML=`<input type="checkbox" class="admin-skill" value="${escapeHtml(s)}" ${checked}> ${escapeHtml(s)}`;wrap.appendChild(lbl);}); }
  ...
  function initUserSelect(){
    ...
    sel.onchange=()=>{
      ...
      if(key==='__alla__'){
        ...
        document.getElementById('persPanel_idag').style.display='none';
        selectPersonalTab('schema');
        if(tabIdag) tabIdag.style.display='none';
        return;
      }
      ...
      if(vol){vol.checked=!!u.isVoluntar;vol.disabled=(u.name==='Adam'||u.name==='Casper');}
      const delBtn=document.getElementById('deleteUserBtn');
      if(delBtn) delBtn.style.display=(u.name==='Adam'||u.name==='Casper')?'none':'block';
      buildAdminSkillsOptions(u.name);
      renderSchemaForEmployee();
    };
    sel.onchange();
  }

  function deleteSchemaDate(date){
    const p=loadShiftPlan();
    const saved=p[date]?JSON.parse(JSON.stringify(p[date])):null;
    delete p[date];
    saveShiftPlan(p);
    renderSchemaAll();
    showUndoToast(`Schema för ${date} borttaget`,()=>{
      if(saved){const q=loadShiftPlan();q[date]=saved;saveShiftPlan(q);}
      renderSchemaAll();
    });
  }
  ...
  function renderSchemaAll(){
    const el=document.getElementById('schemaEmployeeRows');if(!el) return;
    const saveRow=document.querySelector('#persPanel_schema .row');if(saveRow) saveRow.style.display='none';
    const today=todayStr();
    const plan=loadShiftPlan();
    const allOpenDates=loadOpenDates().map(e=>typeof e==='string'?{date:e,hours:''}:e);
    const pastPlanDates=Object.keys(plan).filter(d=>d<today&&Object.values(plan[d]||{}).some(v=>v.hours||v.task));
    const hoursMap=Object.fromEntries(allOpenDates.map(e=>[e.date,e.hours]));
    const hist=loadSalesHistory();
    pastPlanDates.forEach(d=>{if(!hoursMap[d]&&hist[d]?.hours)hoursMap[d]=hist[d].hours;});
    function fmtDate(d){const[,m,day]=d.split('-');const names=['sön','mån','tis','ons','tor','fre','lör'];const wd=new Date(d).getDay();return `${names[wd]} ${parseInt(day)}/${parseInt(m)}`;}
    const upcoming=[{date:today,hours:hoursMap[today]||openingHours||''},...allOpenDates.filter(e=>e.date>today)].filter((e,i,arr)=>arr.findIndex(x=>x.date===e.date)===i).sort((a,b)=>a.date.localeCompare(b.date));
    const past=pastPlanDates.map(d=>({date:d,hours:hoursMap[d]||''})).sort((a,b)=>b.date.localeCompare(a.date));
    const upcomingOpts=upcoming.map(e=>`<option value="${escapeHtml(e.date)}"${e.date===today?' selected':''}>${e.date===today?'Idag':fmtDate(e.date)}${e.hours?' — '+escapeHtml(e.hours):''}</option>`).join('');
    const pastOpts=past.map(e=>`<option value="${escapeHtml(e.date)}">${fmtDate(e.date)}${e.hours?' — '+escapeHtml(e.hours):''}</option>`).join('');
    const dateOpts=pastOpts?`<optgroup label="Kommande">${upcomingOpts}</optgroup><optgroup label="Tidigare">${pastOpts}</optgroup>`:upcomingOpts;
    ...
    el.innerHTML=`<div style="margin-bottom:14px;"><select class="input" id="allaDateSelect" style="width:100%;">${dateOpts}</select></div><div id="allaSchemaList">${listForDate(today)}</div>`;
    document.getElementById('allaDateSelect')?.addEventListener('change',(e)=>{
      const list=document.getElementById('allaSchemaList');
      if(list) list.innerHTML=listForDate(e.target.value);
    });
  }

  document.getElementById('saveAdminBtn')?.addEventListener('click',()=>{
    const key=document.getElementById('userSelect')?.value||'';const u=users[key];if(!u) return;
    ensureProfileDefaults(u.name);
    const accEl=document.getElementById('accessCheckbox');
    const inslEl=document.getElementById('insleppCheckbox');
    if(u.name==='Adam'||u.name==='Casper'){if(accEl) accEl.checked=true;u.hasAccess=true;}else{u.hasAccess=!!(accEl&&accEl.checked);}
    if(u.name==='Casper'){if(inslEl) inslEl.checked=true;u.hasInslepp=true;}else{u.hasInslepp=!!(inslEl&&inslEl.checked);}
    const volEl=document.getElementById('voluntarCheckbox');
  ...
  function openAddUserPanel(){
    ...
    document.getElementById('persTabRow').style.display='none';
    document.getElementById('persPanel_idag').style.display='none';
    document.getElementById('persPanel_schema').style.display='none';
    document.getElementById('persPanel_laggTill').style.display='block';
  }
  function closeAddUserPanel(){
    document.getElementById('persPanel_laggTill').style.display='none';
    document.getElementById('userSelectSection').style.display='';
    document.getElementById('persTabRow').style.display='flex';
    document.getElementById('newUserName').value='';
    document.getElementById('newUserPass').value='';
    document.getElementById('newUserAccessCheckbox').checked=false;
    document.getElementById('newUserInsleppCheckbox').checked=false;
    const warn=document.getElementById('newUserPassWarn');if(warn){warn.textContent='';warn.style.display='none';}
    selectPersonalTab('idag');
  }
  function openDeleteUserModal(){
    const sel=document.getElementById('userSelect');
    const u=users[sel?.value];if(!u) return;
    document.getElementById('deleteUserModalMsg').textContent=`Är du säker på att du vill ta bort ${u.name}?`;
  ...
  function executeDeleteUser(){
    ...
    const plan=loadShiftPlan();
    const savedPlan={};
    Object.keys(plan).forEach(date=>{ if(plan[date][name]!==undefined){ savedPlan[date]=plan[date][name]; delete plan[date][name]; } });
    saveUsers();
    saveUserData();
    saveShiftPlan(plan);
  ...
  (function bindUserMgmt(){
    const root=document.getElementById('adminSettings');if(!root||root._boundUserMgmt) return;
  ...
  async function boot(){
    ...
    try{
      ...
      if(u){loggedInKey=pw;loggedInUser=u.name;isAdmin=(pw==='spök123')?true:!!u.admin;currentHasAccess=!!u.hasAccess||u.name==='Adam'||u.name==='Casper';currentHasInslepp=!!u.hasInslepp||u.name==='Adam'||u.name==='Casper';gtag('set','user_properties',{user_type:'personal'});}
      else{gtag('set','user_properties',{user_type:'guest'});}
    }catch(_e){}
    ...
    if(loggedInUser && localStorage.getItem('kassaLockedV1')){
      ...
      selectMenu(isAdmin?'admin':'profile'); syncDisplayOwnership();
      lockKassa();
    } else if(loggedInUser){ try{const _t=localStorage.getItem('userTheme_'+loggedInUser);if(_t) applyProfileTheme(_t);}catch(_e){} selectMenu(isAdmin?'admin':'profile'); syncDisplayOwnership(); }
    else{
      document.getElementById('welcomeMessage').style.display='flex';
  ...
  function startCustomerDisplayMode(){
    ...
    if(logoEl){ logoEl.src=logoData||'korpen.png'; logoEl.style.display='block'; }
